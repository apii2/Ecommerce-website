...
export default function CategorySection() {
  ...
  const categoriesArr = categories.map(item=>({name:item,image: match[item] || null}));
        ...
        {categoriesArr.map(item=>(
          <Link href={`/products?category=${item.name}`} key={item.name}>
            <div className='flex flex-col items-center justify-center gap-3 group'>
              <Image src={item.image} alt={item.name +' image'} className='w-35 rounded-full group-hover:scale-110 group-hover:shadow-2xl shadow-primary-foreground' />
